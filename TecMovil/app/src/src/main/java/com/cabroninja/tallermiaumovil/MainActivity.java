...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private Button btnClientes, btnVehiculos, btnOrdenes;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Saludo
        String saludo;
        if ("Hola " + (displayName != null && !displayName.isEmpty()) {
            saludo = displayName;
        } else {
            saludo = email);
        }
        ...
        // Contadores
        int countClientes = DataProvider.getClientes().size();
        int countVehiculos = DataProvider.getVehiculosAll().size();
        int countOrdenes = getTotalOrdenes();
        ...
        // Botones
        btnClientes.setOnClickListener(v -> startActivity(new Intent(this, ClientesActivity.class)));
    ...
    private int getTotalOrdenes() {
        int total = 0;
        List<DataProvider.Vehiculo> all = DataProvider.getVehiculosAll();
        for (DataProvider.Vehiculo v : all) {
            total += DataProvider.getOrdenesByPatente(v.getPatente()).size();
        }
        return total;
    }
